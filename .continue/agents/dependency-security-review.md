...
## name: Dependency Security Review description: Review dependency changes for security implications and breaking changes
...
1. **Major version bumps** - For major version upgrades:

   - Are there breaking changes that affect our usage?
   - Have the callers been updated to match the new API?

1. **Removed dependencies** - For each removed dependency:
...
1. **New transitive dependencies** - Check if the total dependency count increased significantly.
...
1. **Native/binary dependencies** - New native modules (`node-gyp`, `.node` binaries) increase the attack surface and build complexity.

1. **Post-install scripts** - Dependencies with `postinstall` scripts can execute arbitrary code during `npm install`.
